...
module Bgit::Accounting
  RSpec.describe Contacts::PhoneNumber, type: :model do
    describe "associations" do
      it { expect(subject).to belong_to(:company) }
    end

    describe "validations" do
      subject { build(:bgit_accounting_contacts_phone_number) }
      ...
      it { expect(subject).to validate_inclusion_of(:role).in_array(%w[business office mobile private fax other]) }
    end
  end
end
